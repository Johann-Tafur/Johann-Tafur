<!--
CONFIG
FULL_NAME: Johann Tafur Farfán
GITHUB_USER: Johann-Tafur
EMAIL: johanntafurfarfan@gmail.com
CODE_SENA_URL: https://github.com/code-sena
PORTFOLIO: WAYLIX - Smart Urban Mobility
-->

<div align="center">

<img width="100%" src="./assets/johann-neon-systems-banner.svg" alt="Johann Tafur — Neon systems" />

<img src="https://capsule-render.vercel.app/api?type=rect&height=112&color=0:090014,22:6A00FF,52:FF00C8,76:00E5FF,100:B8FF00&text=JOHANN%20TAFUR%20FARFAN&fontSize=40&fontColor=FFFFFF&fontAlignY=43&desc=Full-Stack%20Developer%20%7C%20Java%20%7C%20Smart%20Urban%20Mobility&descAlignY=70&descAlign=50" alt="Johann Tafur Farfán Banner" />

<br />

<img src="https://readme-typing-svg.demolab.com?font=Space+Mono&weight=800&size=22&pause=800&color=00E5FF&center=true&vCenter=true&width=900&lines=%E2%9A%A1+SOFTWARE+DEVELOPER+IN+TRAINING;%F0%9F%9A%8D+BUILDING+WAYLIX;%F0%9F%8C%8F+SMART+URBAN+MOBILITY;%F0%9F%92%BB+JAVA+%7C+MYSQL+%7C+FULL-STACK" alt="Typing animation" />

<br />

[![GitHub](https://img.shields.io/badge/@Johann--Tafur-090014?style=for-the-badge&logo=github&logoColor=00E5FF)](https://github.com/Johann-Tafur)
[![ADSO](https://img.shields.io/badge/ADSO_3239137-FF00C8?style=for-the-badge&logoColor=white)](https://www.sena.edu.co/)
[![Neiva](https://img.shields.io/badge/NEIVA%2C_COLOMBIA-B8FF00?style=for-the-badge&logo=googlemaps&logoColor=090014)](https://www.google.com/maps/search/?api=1&query=Neiva%2C%20Colombia)
[![Email](https://img.shields.io/badge/EMAIL-00E5FF?style=for-the-badge&logo=gmail&logoColor=090014)](mailto:johanntafurfarfan@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LINKEDIN-6A00FF?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/johann-tafur)

</div>

---

## ◢◤ WHO IS ON THE FREQUENCY?

<table align="center">
  <tr>
    <td>

**Identity**
> Johann Tafur Farfán

**Role**
> Software Developer in Training

**Base**
> Neiva, Huila — Colombia

**Current Quest**
> WAYLIX (Smart Urban Mobility)

**Mission**
> Convertir problemas reales en experiencias digitales útiles

    </td>
    <td>

> *Del sector farmacéutico al software:*  
> **Precisión, orden y personas primero.**
> 
> No quiero solo escribir código;  
> quiero construir cosas que le sirvan a alguien.

    </td>
  </tr>
</table>

<div align="center">

### **✨ THE FUTURE IS NOT LOADING. I AM BUILDING IT. ✨**

</div>

---

## ◢◤ TECH STACK // PRODUCTION READY

<div align="center">

<img src="https://skillicons.dev/icons?i=java,mysql,html,css,js,git,github,docker,vscode,linux&theme=dark" alt="Java, MySQL, HTML, CSS, JavaScript, Git, GitHub, Docker, VS Code, Linux" />

<br /><br />

| **⚡ Backend** | **🗄️ Database** | **🎨 Frontend** | **🔧 DevOps** |
|:--:|:--:|:--:|:--:|
| ![Java](https://img.shields.io/badge/JAVA-6A00FF?style=for-the-badge&logo=openjdk&logoColor=white) | ![SQL](https://img.shields.io/badge/SQL-00E5FF?style=for-the-badge&logo=mysql&logoColor=090014) | ![HTML](https://img.shields.io/badge/HTML5-FF6B35?style=for-the-badge&logo=html5&logoColor=white) | ![Docker](https://img.shields.io/badge/DOCKER-2496ED?style=for-the-badge&logo=docker&logoColor=white) |
| OOP · Design Patterns | Relational Design | HTML · CSS · Responsive | Linux · Git · GitHub |

</div>

### 🚀 EN PRODUCCIÓN
- ✅ **Backend:** Java, POO, lógica de negocio, patrones de diseño
- ✅ **Database:** SQL/MySQL, diseño relacional, optimización
- ✅ **Frontend:** HTML5, CSS3, JavaScript vanilla
- ✅ **DevOps:** Docker, Git, control de versiones, Linux

### 🧬 PRÓXIMA MUTACIÓN
- 🔄 Spring Boot & arquitectura escalable
- 🔄 Microservicios y APIs REST
- 🔄 Full-stack con frameworks modernos
- 🔄 Código limpio y SOLID principles

---

## 🚍 MAIN QUEST // WAYLIX

<div align="center">

### *La ciudad se mueve. WAYLIX la hace entendible.*

**Movilidad urbana inteligente para conectar pasajeros, conductores y empresas en tiempo real.**

```
┌─────────────────────────────────────────────────┐
│                                                 │
│  PASAJERO  ──►  RUTA IDEAL  ──►  BUS EN VIVO  │
│     ▲                               │           │
│     │         ┌──────────┐          ▼           │
│     └─────────┤  WAYLIX  │◄── OCUPACIÓN + TF  │
│              └──────────┘                       │
│                                                 │
└─────────────────────────────────────────────────┘
```

</div>

### 📍 Características Principales

| Característica | Beneficio |
|:--|:--|
| 🗺️ **Rutas + Paradas + ETA** | Navega sin sorpresas |
| 👥 **Ocupación en Vivo** | Ve el bus antes de subirte |
| 🚦 **Alertas de Tráfico** | Retrasos, fallas, novedades en tiempo real |
| 🧭 **Recomendaciones Smart** | Llega mejor, no solo llegas |
| 🏢 **Coordinación B2B** | Herramientas para empresas y conductores |

> **¿Por qué la movilidad urbana debería sentirse como adivinar?**  
> WAYLIX nace para cambiar esa experiencia. Hoy no existe. Mañana va a existir.

---

## 📊 LIVE TELEMETRY

<div align="center">

<a href="https://github.com/Johann-Tafur">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=Johann-Tafur&show_icons=true&hide_border=true&bg_color=090014&title_color=FF00C8&text_color=F8F5FF&icon_color=00E5FF&ring_color=6A00FF&include_all_commits=true&count_private=true" alt="GitHub Stats" />
</a>

<a href="https://github.com/Johann-Tafur">
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Johann-Tafur&layout=compact&hide_border=true&bg_color=090014&title_color=00E5FF&text_color=F8F5FF&card_width=320&langs_count=8" alt="Top Languages" />
</a>

<br /><br />

<img src="https://streak-stats.demolab.com?user=Johann-Tafur&hide_border=true&background=090014&ring=FF00C8&fire=B8FF00&currStreakLabel=00E5FF&sideLabels=F8F5FF&dates=9D8CFF&stroke=24114D" alt="GitHub Streak Stats" />

<br /><br />

<img width="96%" src="https://github-readme-activity-graph.vercel.app/graph?username=Johann-Tafur&bg_color=090014&color=00E5FF&line=FF00C8&point=B8FF00&area=true&hide_border=true&grid=false" alt="Contribution Graph" />

<br /><br />

### 🐍 NEON TRAIL

<img src="https://raw.githubusercontent.com/Johann-Tafur/Johann-Tafur/output/github-contribution-grid-snake-dark.svg" alt="Animated neon snake across contribution grid" />

</div>

---

## 🎯 CURRENTLY LEARNING

<div align="center">

| Topic | Progress | Status |
|:--|:--|:--|
| Spring Boot & Enterprise Patterns | 🔄 | In Progress |
| System Design & Architecture | 🔄 | Exploring |
| REST APIs & Microservices | 🔄 | Practicing |
| Clean Code & SOLID Principles | 🔄 | Mastering |
| Full-Stack Development | 🔄 | Building |

</div>

---

## 📚 EDUCATION & CERTIFICATIONS

<details>
  <summary><b>🎓 Estudios Académicos</b></summary>

  - **ADSO (Análisis y Desarrollo de Software)**  
    SENA · Ficha 3239137 · 2024-2026 ✅ En curso
  
  - **Ingeniería de Software**  
    UNIMINUTO · En curso

  - **Bootcamp Development**  
    CODE SENA · Specialized Training
    
</details>

---

## 💼 HOW TO REACH ME

<div align="center">

### ¿Construimos algo que todavía no existe?

**Estoy abierto a:**
- 🤝 Colaboraciones en proyectos tech
- 💡 Discusiones sobre arquitectura y diseño
- 📖 Mentoring y aprendizaje continuo
- 🚀 Oportunidades en startups y tech
- 🤖 Open source contributions

<br />

[![Explorar Repositorios](https://img.shields.io/badge/EXPLORAR_REPOSITORIOS-FF00C8?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Johann-Tafur?tab=repositories)
[![Enviar Email](https://img.shields.io/badge/ENVIAR_SEÑAL-00E5FF?style=for-the-badge&logo=gmail&logoColor=090014)](mailto:johanntafurfarfan@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LINKEDIN-6A00FF?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/johann-tafur)

<br /><br />

<img src="https://capsule-render.vercel.app/api?type=waving&height=150&color=0:090014,35:6A00FF,65:FF00C8,100:00E5FF&section=footer&text=SYSTEM%20ONLINE%20%7C%20READY%20TO%20BUILD&fontColor=FFFFFF&fontSize=26" alt="Footer wave" />

<sub>

**TAFUR // NEIVA // 2026**

*Built with code, curiosity and controlled chaos.*  
*Convirtiendo problemas reales en soluciones digitales.*

</sub>

</div>

---

<div align="center">

### 🌟 *If you made it this far, you're already part of the mission.* 🌟

</div>
